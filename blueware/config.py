...
import blueware.core.agent
import blueware.core.config

import blueware.api.settings
import blueware.api.import_hook
import blueware.api.exceptions
import blueware.api.web_transaction
import blueware.api.background_task
import blueware.api.database_trace
import blueware.api.external_trace
import blueware.api.function_trace
import blueware.api.generator_trace
import blueware.api.profile_trace
import blueware.api.memcache_trace
import blueware.api.transaction_name
import blueware.api.error_trace
import blueware.api.function_profile
import blueware.api.application

import blueware.console
...
sys.meta_path.insert(0, blueware.api.import_hook.ImportHookFinder())
...
_settings = blueware.api.settings.settings()
...
_RECORD_SQL = {
    "off": blueware.api.settings.RECORDSQL_OFF,
    "raw": blueware.api.settings.RECORDSQL_RAW,
    "obfuscated": blueware.api.settings.RECORDSQL_OBFUSCATED,
}
...
def _map_labels(s):
    return blueware.core.config._environ_as_mapping(name='', default=s)
...
def _map_rum_request_ip(s):
    return s.split()

def _map_rum_request_params(s):
    return s.split()
...
def _merge_ignore_status_codes(s):
    return blueware.core.config._parse_ignore_status_codes(
...
def _raise_configuration_error(section, option=None):
    ...
    if option is None:
        ...
        if not _ignore_errors:
            if section:
                raise blueware.api.exceptions.ConfigurationError(
                        'Invalid configuration for section "%s". '
                        'Check blueware agent log file for further '
                        'details.' % section)
            else:
                raise blueware.api.exceptions.ConfigurationError(
                        'Invalid configuration. Check blueware agent '
        ...
        if not _ignore_errors:
            if section:
                raise blueware.api.exceptions.ConfigurationError(
                        ...
                        'section "%s". Check blueware agent log '
...
def _process_configuration(section):
    ...
    _process_setting(section, 'browser_monitoring.enabled',
                     'getboolean', None)
    _process_setting(section, 'browser_monitoring.transform',
                     'getboolean', None)
    ...
    _process_setting(section, 'browser_monitoring.capture_attributes',
                     'getboolean', None)
    _process_setting(section, 'browser_monitoring.beacon_host',
                     'get', None)
    _process_setting(section, 'browser_monitoring.beacon_port',
                     'getint', None)
    _process_setting(section, 'browser_monitoring.apdex_t',
                     'getfloat', None),
    _process_setting(section, 'browser_monitoring.content_type',
                     'get', _map_browser_monitoring_content_type),
    _process_setting(section, 'browser_monitoring.is_js_text',
                     'getboolean', None),
    _process_setting(section, 'browser_monitoring.request_ip',
                     'get', _map_rum_request_ip),
    _process_setting(section, 'browser_monitoring.request_url',
                     'get', None),
    _process_setting(section, 'browser_monitoring.request_params',
                     'get', _map_rum_request_params),
...
def _process_app_name_setting():
    ...
    if linked:
        blueware.api.application.Application.run_on_initialization(
...
def _load_configuration(config_file=None, environment=None,
        ...
        for flag in _settings.feature_flag:
            if flag not in _FEATURE_FLAGS:
                _logger.warning('Unknown agent feature flag %r provided. '
                        ...
                        'contact blueware support for clarification of '
    ...
    _process_setting('blueware', 'log_file', 'get', None)

    if environment:
        _process_setting('blueware:%s' % environment,
                         'log_file', 'get', None)

    if log_file is None:
        log_file = _settings.log_file

    _process_setting('blueware', 'log_level', 'get', _map_log_level)
    ...
    _process_configuration('blueware')
    ...
    if environment:
        ...
        _process_configuration('blueware:%s' % environment)
    ...
    for function in _settings.transaction_tracer.function_trace:
        try:
            ...
            blueware.api.import_hook.register_import_hook(module, hook)
        except Exception:
...
def _raise_instrumentation_error(type, locals):
    ...
    if not _ignore_errors:
        raise blueware.api.exceptions.InstrumentationError(
                'Failure when instrumenting code. Check blueware '
...
def _module_import_hook(target, module, function):
    def _instrument(target):
        ...
        try:
            instrumented = target._bw_instrumented
        except AttributeError:
            instrumented = target._bw_instrumented = set()
        ...
        try:
            getattr(blueware.api.import_hook.import_module(module),
                    function)(target)
    ...
    for section in _config_object.sections():
        ...
        try:
            ...
            if target not in _module_import_hook_registry:
                ...
                blueware.api.import_hook.register_import_hook(target, hook)
...
def _wsgi_application_import_hook(object_path, application):
    def _instrument(target):
        ...
        try:
            blueware.api.web_transaction.wrap_wsgi_application(
                    target, object_path, application)
        except Exception:
...
def _background_task_import_hook(object_path, application, name, group):
    def _instrument(target):
        ...
        try:
            blueware.api.background_task.wrap_background_task(
                    target, object_path, application, name, group)
        except Exception:
...
def _database_trace_import_hook(object_path, sql):
    def _instrument(target):
        ...
        try:
            blueware.api.database_trace.wrap_database_trace(
                    target, object_path, sql)
        except Exception:
...
def _external_trace_import_hook(object_path, library, url, method):
    def _instrument(target):
        ...
        try:
            blueware.api.external_trace.wrap_external_trace(
                    target, object_path, library, url, method)
        except Exception:
...
def _function_trace_import_hook(object_path, name, group, label, params,
    ...
    def _instrument(target):
        ...
        try:
            blueware.api.function_trace.wrap_function_trace(
                    target, object_path, name, group, label, params,
                    terminal, rollup)
        except Exception:
...
def _generator_trace_import_hook(object_path, name, group):
    def _instrument(target):
        ...
        try:
            blueware.api.generator_trace.wrap_generator_trace(
                    target, object_path, name, group)
        except Exception:
...
def _profile_trace_import_hook(object_path, name, group, depth):
    def _instrument(target):
        ...
        try:
            blueware.api.profile_trace.wrap_profile_trace(
...
def _memcache_trace_import_hook(object_path, command):
    def _instrument(target):
        ...
        try:
            blueware.api.memcache_trace.wrap_memcache_trace(
                    target, object_path, command)
        except Exception:
...
def _transaction_name_import_hook(object_path, name, group, priority):
    def _instrument(target):
        ...
        try:
            blueware.api.transaction_name.wrap_transaction_name(
                    target, object_path, name, group, priority)
        except Exception:
...
def _error_trace_import_hook(object_path, ignore_errors):
    def _instrument(target):
        ...
        try:
            blueware.api.error_trace.wrap_error_trace(
                    target, object_path, ignore_errors)
        except Exception:
...
def _startup_data_source():
    ...
    agent_instance = blueware.core.agent.agent_instance()

    for section, module, object_path, application, name, \
            settings, properties in _data_sources:
        try:
            source = getattr(blueware.api.import_hook.import_module(
                    module), object_path)
...
def _setup_data_source():
    ...
    if _data_sources:
        blueware.core.agent.Agent.run_on_startup(_startup_data_source)
...
def _function_profile_import_hook(object_path, filename, delay, checkpoint):
    def _instrument(target):
        ...
        try:
            blueware.api.function_profile.wrap_function_profile(target,
...
def _process_module_definition(target, module, function='instrument'):
    ...
    try:
        if enabled and not execute:
            ...
            blueware.api.import_hook.register_import_hook(target,
...
def _process_module_builtin_defaults():
    ...
    _process_module_definition('bottle',
            'blueware.hooks.framework_bottle',
            'instrument_bottle')

    _process_module_definition('cherrypy._cpreqbody',
            'blueware.hooks.framework_cherrypy',
    ...
    _process_module_definition('psycopg2ct',
            'blueware.hooks.database_psycopg2ct',
    ...
    _process_module_definition('psycopg2cffi',
            'blueware.hooks.database_psycopg2cffi',
    ...
    _process_module_definition('postgresql.driver.dbapi20',
            'blueware.hooks.database_postgresql',
    ...
    _process_module_definition('sqlite3',
            'blueware.hooks.database_sqlite',
            'instrument_sqlite3')
    ...
    if six.PY2:
        _process_module_definition('urllib2',
                'blueware.hooks.external_urllib2')

    _process_module_definition('urllib3.connectionpool',
            'blueware.hooks.external_urllib3',
    ...
    _process_module_definition('requests.api',
            'blueware.hooks.external_requests',
    ...
    _process_module_definition('redis.connection',
            'blueware.hooks.datastore_redis',
    ...
    _process_module_definition('motor',
            'blueware.hooks.datastore_motor', 'patch_motor')

    _process_module_definition('piston.resource',
            'blueware.hooks.component_piston',
    ...
    _process_module_definition('tastypie.resources',
            'blueware.hooks.component_tastypie',
    ...
    _process_module_definition('rest_framework.views',
            'blueware.hooks.component_djangorestframework',
            'instrument_rest_framework_views')

    _process_module_definition('celery.task.base',
            'blueware.hooks.application_celery',
    ...
    _process_module_definition('cornice.service',
            'blueware.hooks.component_cornice',
            'instrument_cornice_service')

    _process_module_definition('twisted.web.server',
            'blueware.hooks.framework_twisted.web.server',
            'instrument_twisted_web_server')
    _process_module_definition('twisted.web.http',
            'blueware.hooks.framework_twisted.web.http',
            'instrument_twisted_web_http')
    _process_module_definition('twisted.web.resource',
            'blueware.hooks.framework_twisted.web.resource',
            'instrument_twisted_web_resource')
    _process_module_definition('twisted.web.wsgi',
            'blueware.hooks.framework_twisted.web.wsgi',
            'instrument_twisted_web_wsgi')
    _process_module_definition('twisted.internet.defer',
            'blueware.hooks.framework_twisted.internet.defer',
            'instrument_twisted_internet_defer')
    # _process_module_definition('twisted.internet.reactor',
    #         'blueware.hooks.framework_twisted.internet.reactor',
    #         'instrument_twisted_internet_reactor')

    _process_module_definition('cyclone.httpserver',
            'blueware.hooks.framework_cyclone.httpserver',
            'instrument_cyclone_httpserver')
    _process_module_definition('cyclone.httputil',
            'blueware.hooks.framework_cyclone.httputil',
            'instrument_cyclone_httputil')
    _process_module_definition('cyclone.web',
            'blueware.hooks.framework_cyclone.web',
            'instrument_cyclone_web')
    _process_module_definition('cyclone.template',
            'blueware.hooks.framework_cyclone.template',
            'instrument_cyclone_template')
    _process_module_definition('cyclone.redis',
            'blueware.hooks.framework_cyclone.redis',
            'instrument_cyclone_redis')

    _process_module_definition('gevent.monkey',
            'blueware.hooks.coroutines_gevent',
            'instrument_gevent_monkey')

    _process_module_definition('openerp.service.wsgi_server',
            'blueware.hooks.framework_openerp',
            'instrument_openerp_service_wsgi_server')
    _process_module_definition('openerp.http',
            'blueware.hooks.framework_openerp',
            'instrument_openerp_http')
    _process_module_definition('openerp.modules.registry',
            'blueware.hooks.framework_openerp',
            'instrument_openerp_modules_registry')

    _process_module_definition('weberror.errormiddleware',
            'blueware.hooks.middleware_weberror',
    ...
    _process_module_definition('thrift.transport.TSocket',
            'blueware.hooks.external_thrift')

    _process_module_definition('gearman.client',
            'blueware.hooks.application_gearman',
...
def _startup_agent_console():
    ...
    _console = blueware.console.ConnectionManager(
            _settings.console.listener_socket)

def _setup_agent_console():
    if _settings.console.listener_socket:
        blueware.core.agent.Agent.run_on_startup(_startup_agent_console)

def initialize(config_file=None, environment=None, ignore_errors=None,
            log_file=None, log_level=None):

    if config_file is None:
        config_file = os.environ.get('BLUEWARE_CONFIG_FILE', None)

    if environment is None:
        environment = os.environ.get('BLUEWARE_ENVIRONMENT', None)

    if ignore_errors is None:
        ignore_errors = blueware.core.config._environ_as_bool(
                'BLUEWARE_IGNORE_STARTUP_ERRORS', True)
...
def filter_app_factory(app, global_conf, config_file, environment=None):
    ...
    return blueware.api.web_transaction.WSGIApplicationWrapper(app)
