...
class _ObjectWrapperBase(object):
    ...
    def __setattr__(self, name, value):
        if name.startswith('_bw_'):
            name = name.replace('_bw_', '_self_', 1)
    ...
    @property
    def _bw_next_object(self):
        return self.__wrapped__

    @property
    def _bw_last_object(self):
        try:
            return self._self_last_object
        except AttributeError:
            self._self_last_object = getattr(self.__wrapped__,
                    '_bw_last_object', self.__wrapped__)
            return self._self_last_object

    @property
    def _bw_instance(self):
        return self._self_instance

    @property
    def _bw_wrapper(self):
...
class _OABoundFunctionWrapper(_ObjectWrapperBase, _BoundFunctionWrapper):
    pass
