...
def _object_context_py3(object):
    ...
    # qualified name. Please notice that __qualname__ was added
    # in python3.3 referenced to PEP 3155.
    ...
    if path is None and hasattr(object, '__class__'):
        if sys.version_info[1] > 2:
            ...
            if hasattr(object, '__self__'):
                path = '.'.join([getattr(object.__self__, '__name__', ''), getattr(object, '__name__', '')])
            else:
                path = object.__name__
...
def object_context(target):
    ...
    details = getattr(target, '_bw_object_path', None)
    ...
    parent = getattr(target, '_bw_parent', None)

    if parent:
        details = getattr(parent, '_bw_object_path', None)
    ...
    # attribute _bw_last_object refers to the wrapped object
    ...
    source = getattr(target, '_bw_last_object', None)
    ...
    try:
        ...
        if target is not source:
            ...
            if parent:
                parent._bw_object_path = details
            ...
            target._bw_object_path = details
        ...
        source._bw_object_path = details
