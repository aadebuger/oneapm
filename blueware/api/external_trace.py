...
class ExternalTrace(TimeTrace):
    ...
    def process_response_headers(self, response_headers):
        ...
        try:
            for k, v in response_headers:
                if k.upper() == 'X-BLUEWARE-APP-DATA':
                    appdata = json_decode(deobfuscate(v,
                            self.settings.encoding_key))
                    break

            # app data fields,  according to web_txn craft
            # [
            #  cross_process_id,
            #  txn_name,
            #  queue_time,
            ...
            if appdata:
                ...
                self.params['external_exclusive'] = appdata[3]
    ...
    def generate_request_headers(transaction):
        """
        Return a list of BLUEWARE specific headers as tuples
        [(HEADER_NAME0, HEADER_VALUE0), (HEADER_NAME1, HEADER_VALUE1)]
        ...
        if settings.cross_application_tracer.enabled:
            ...
            bw_headers.append(('X-BLUEWARE-ID', encoded_cross_process_id))
            ...
            encoded_transaction = obfuscate(json_encode(transaction_data),
                    settings.encoding_key)
            bw_headers.append(('X-BLUEWARE-Transaction', encoded_transaction))

        if transaction.synthetics_header:
            bw_headers.append(
                ('X-BLUEWARE-Synthetics', transaction.synthetics_header))
