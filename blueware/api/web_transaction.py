...
_js_agent_header_fragment = '<script type="text/javascript" src="%s"></script>'
_js_agent_footer_fragment = '<script type="text/javascript">'\
                            'window.BWEUM||(BWEUM={});BWEUM.info=%s</script>'
...
def _extract_token(cookie):
    try:
        t = re.search(r"\bBWAGENT=(tk=.{16})", cookie)
...
def _check_operate_condition(operate, source, dest):
    if source is None or dest is None:
        return False

    operate_check = {
        'Equals': source == dest,
        'In': dest in source.split(','),
        'Contains': source in dest,
        'StartsWith': dest.startswith(source),
        'EndWith': dest.endswith(source),
        'Existence': dest is not None
    }

    return operate_check[operate]

def _get_url(environ, request_uri):
...
class WebTransaction(Transaction):
    ...
    def __init__(self, application, environ):
        ...
        # the value of the variable "blueware.enabled"
        ...
        self.background_task = _lookup_environ_setting(environ,
                'blueware.set_background_task', False)

        self.ignore_transaction = _lookup_environ_setting(environ,
                'blueware.ignore_transaction', False)
        self.suppress_apdex = _lookup_environ_setting(environ,
                'blueware.suppress_apdex_metric', False)
        self.suppress_transaction_trace = _lookup_environ_setting(environ,
                'blueware.suppress_transaction_trace', False)
        self.capture_params = _lookup_environ_setting(environ,
                'blueware.capture_request_params',
                settings.capture_params)
        self.autorum_disabled = _lookup_environ_setting(environ,
                'blueware.disable_browser_autorum',
                not getattr(settings.browser_monitoring, 'transform'))

        # Strict autorum insert condition.
        self.autorum_ip = _lookup_environ_setting(environ,
                'blueware.autorum_ip',
                getattr(settings.browser_monitoring, 'request_ip'))
        self.autorum_params = _lookup_environ_setting(environ,
                'blueware.autorum_params',
                getattr(settings.browser_monitoring, 'request_params'))
        # To get String or None instead of a Boolean.
        self.autorum_url = getattr(settings.browser_monitoring, 'request_url')
        ...
        if http_cookie and ("BWAGENT" in http_cookie):
        ...
        try:
            _application = application._agent.application(application.name)

            if _application.active and hasattr(_application, 'btm_url_match') and \
                    _application.btm_url_match:
                url = _get_url(environ, self._request_uri)

                for url_match, url_rules in _application.btm_url_match.items():
                    if url_match(environ.get('REQUEST_METHOD'), url):
                        ...
                        for rule in url_rules:
                            ...
                            for expression in match_expression:
                                if expression['name'].startswith('parameter'):
                                    param_name = expression['name'][expression['name'].find('.')+1:]
                                    param_value = expression['match_value']
                                    request_param = self._request_params.get(param_name) and \
                                                    self._request_params.get(param_name)[0]

                                    if not _check_operate_condition(expression['operate'],
                                                                    param_value,
                                                                    request_param):
                                        condition_satisfied = False
                                        break

                                if expression['name'].startswith('header'):
                                    header_name = expression['name'][expression['name'].find('.')+1:]
                                    header_value = expression['match_value']

                                    if not _check_operate_condition(expression['operate'],
                                                                    header_value,
                                                                    environ.get(header_name)):
                                        condition_satisfied = False
                                        break

                            if condition_satisfied and isinstance(rule['priority'], int):
                                # guarantee the priority is greater than the agent internal priority
                                rule['priority'] += 6
                                self.set_transaction_name(rule['replacement'],
                                                          group='BT',
        ...
        if settings.synthetics.enabled and \
            ...
            try:
                header_name = 'HTTP_X_BLUEWARE_SYNTHETICS'
                header = self.decode_blueware_header(environ, header_name)
        ...
        # Check for the blueware cross process ID header and extract
        ...
        if settings.cross_application_tracer.enabled and \
            ...
            client_cross_process_id = environ.get('HTTP_X_BLUEWARE_ID')
            ...
            if client_cross_process_id:
                try:
                    ...
                    if client_account_id in settings.trusted_account_ids:
                        ...
                        header_name = 'HTTP_X_BLUEWARE_TRANSACTION'
                        txn_header = self.decode_blueware_header(
                                environ, header_name)
    ...
    def decode_blueware_header(self, environ, header_name):
    ...
    def process_response(self, status, response_headers, *args):
        ...
        if self.client_cross_process_id is not None:
            ...
            additional_headers.append(('X-BLUEWARE-App-Data', obfuscate(
    ...
    def browser_timing_header(self):
        ...
        if self._settings.js_agent_loader:
            if self._settings.browser_monitoring.is_js_text:
                header = '<script type="text/javascript">%s</script>' % self._settings.js_agent_loader
            else:
...
def WSGIApplicationWrapper(wrapped, application=None, name=None,
    ...
    def _bw_wsgi_application_wrapper_(wrapped, instance, args, kwargs):
        ...
        app_name = environ.get('blueware.app_name')
        ...
        try:
            ...
            with FunctionTrace(transaction, name='Application',
                ...
                with FunctionTrace(transaction, name=callable_name(wrapped)):
                    # to strict autorum insert accoring ip and url
                    if not transaction.autorum_ip and not transaction.autorum_url:
                        autorum_pass = True
                    else:
                        ip_match = False
                        url_match = False
                        params_match = True
                        if transaction.autorum_ip:
                            ip_match = transaction.autorum_ip and (
                                environ.get('REMOTE_ADDR', '') in transaction.autorum_ip)
                        if transaction.autorum_url:
                            url_match = transaction.autorum_url and (
                                environ.get('PATH_INFO', '') == transaction.autorum_url)
                        if url_match:
                            if transaction.autorum_params:
                                if environ.get('REQUEST_METHOD', '') == 'GET':
                                    request_params = [
                                        x.replace('=', ':') for x in
                                            environ.get('QUERY_STRING', '').split('&')
                                        ]
                                    for x in transaction.autorum_params:
                                        if x not in request_params:
                                            params_match = False
                                else:
                                    for x in transaction.autorum_params:
                                        arg, val = x.split(':', 1)
                                        env_key = 'HTTP_{}'.format(arg.upper())
                                        if environ.get(env_key, '') != val:
                                            params_match = False
                        autorum_pass = ip_match or (url_match and params_match)

                    autorum_enable = (settings and
                        settings.browser_monitoring.enabled and
                        not transaction.autorum_disabled)

                    if autorum_enable and autorum_pass:
    ...
    return FunctionWrapper(wrapped, _bw_wsgi_application_wrapper_)
