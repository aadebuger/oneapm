...
import blueware.core.transaction_node
import blueware.core.database_node
import blueware.core.error_node

from blueware.core.stats_engine import CustomMetrics
from blueware.core.transaction_cache import transaction_cache
from blueware.core.thread_utilization import utilization_tracker
...
class Transaction(object):
        ...
        if global_settings.enabled:
            if enabled or (enabled is None and application.enabled):
                ...
                if not self._settings:
                    ...
                    # print('activate from transaction.')
    ...
    def __exit__(self, exc, value, tb):
        ...
        if not len(self._node_stack) == 1:
            _logger.error('Runtime instrumentation error. Exiting the '
                    ...
                    'Node stack is %r. Report this issue to blueware '
        ...
        node = blueware.core.transaction_node.TransactionNode(
    ...
    def record_exception(self, exc=None, value=None, tb=None,
        ...
        node = blueware.core.error_node.ErrorNode(
                timestamp=time.time(),
                type=fullname,
                message=message,
    ...
    def _pop_current(self, node):
        ...
        try:
            ...
            _logger.error('Runtime instrumentation error. Object on '
                    ...
                    'this issue to blueware support.\n%s', last, node,
    ...
    def _process_node(self, node):
        ...
        if type(node) is blueware.core.database_node.DatabaseNode:
            settings = self._settings
