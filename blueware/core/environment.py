...
try:
    import blueware.core._thread_utilization
except ImportError:
    pass
...
def environment_settings():
    ...
    env.append(('Agent Version', blueware.version))

    if 'BLUEWARE_ADMIN_COMMAND' in os.environ:
        env.append(('Admin Command', os.environ['BLUEWARE_ADMIN_COMMAND']))
        del os.environ['BLUEWARE_ADMIN_COMMAND']
    ...
    if 'blueware.core._thread_utilization' in sys.modules:
        extensions.append('blueware.core._thread_utilization')
    ...
    for name, module in list(sys.modules.items()):
        if name.startswith('blueware.hooks.'):
            plugins.append(name)
