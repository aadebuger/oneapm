...
class StatsEngine(object):
    ...
    @internal_trace('Supportability/Python/StatsEngine/Calls/record_transaction')
    ...
    def create_analytic_event(self, transaction):
        ...
        if transaction.path_hash:
            record['bw.guid'] = transaction.guid
            record['bw.tripId'] = transaction.trip_id
            record['bw.pathHash'] = transaction.path_hash

            _add_if_not_empty('bw.referringPathHash',
                    transaction.referring_path_hash)
            _add_if_not_empty('bw.alternatePathHashes',
                    ','.join(transaction.alternate_path_hashes))
            _add_if_not_empty('bw.referringTransactionGuid',
                    transaction.referring_transaction_guid)
            _add_if_not_empty('bw.apdexPerfZone',
        ...
        if transaction.synthetics_resource_id:
            record['bw.guid'] = transaction.guid
            txn = transaction
            record['bw.syntheticsResourceId'] = txn.synthetics_resource_id
            record['bw.syntheticsJobId'] = txn.synthetics_job_id
            record['bw.syntheticsMonitorId'] = txn.synthetics_monitor_id
    ...
    @internal_trace('Supportability/Python/StatsEngine/Calls/transaction_trace_data')
    def transaction_trace_data(self, connections):
        ...
        if self.__settings.debug.log_transaction_trace_payload:
            _logger.debug('trace_data=%s.', trace_data)
