...
_settings.log_file = os.environ.get('BLUEWARE_LOG', None)
_settings.audit_log_file = os.environ.get('BLUEWARE_AUDIT_LOG', None)
...
_settings.enabled = _environ_as_bool('BLUEWARE_ENABLED', False)

_settings.feature_flag = _environ_as_set('BLUEWARE_FEATURE_FLAG', '')

_settings.log_level = os.environ.get('BLUEWARE_LOG_LEVEL', 'INFO').upper()
...
_settings.license_key = os.environ.get('BLUEWARE_LICENSE_KEY', None)
_settings.api_key = os.environ.get('BLUEWARE_API_KEY', None)

_settings.ssl = _environ_as_bool('BLUEWARE_SSL', True)

_settings.host = os.environ.get('BLUEWARE_HOST', 'tpm.oneapm.com')
_settings.port = int(os.environ.get('BLUEWARE_PORT', '0'))

_settings.proxy_scheme = os.environ.get('BLUEWARE_PROXY_SCHEME', None)
_settings.proxy_host = os.environ.get('BLUEWARE_PROXY_HOST', None)
_settings.proxy_port = int(os.environ.get('BLUEWARE_PROXY_PORT', '0'))
_settings.proxy_user = os.environ.get('BLUEWARE_PROXY_USER', None)
_settings.proxy_pass = os.environ.get('BLUEWARE_PROXY_PASS', None)

_settings.app_name = os.environ.get('BLUEWARE_APP_NAME', 'Python Application')
_settings.linked_applications = []

_settings.process_host.display_name = None

_settings.labels = _environ_as_mapping('BLUEWARE_LABELS', '')

_settings.monitor_mode = _environ_as_bool('BLUEWARE_MONITOR_MODE', True)

_settings.developer_mode = _environ_as_bool('BLUEWARE_DEVELOPER_MODE', False)

_settings.high_security = _environ_as_bool('BLUEWARE_HIGH_SECURITY', False)
...
_settings.startup_timeout = float(
       os.environ.get('BLUEWARE_STARTUP_TIMEOUT', '0.0'))
_settings.shutdown_timeout = float(
       os.environ.get('BLUEWARE_SHUTDOWN_TIMEOUT', '2.5'))
...
_settings.encoding_key = '1111111111111111111111111111111111111111'
...
# _settings.analytics_events.enabled = False
...
_settings.browser_monitoring.transform = True
...
_settings.browser_monitoring.beacon_host = 'tpm.oneapm.com'
_settings.browser_monitoring.beacon_port = 80
_settings.browser_monitoring.apdex_t = 7.0
_settings.browser_monitoring.content_type = ['text/html']
_settings.browser_monitoring.is_js_text = True
_settings.browser_monitoring.request_ip = []
_settings.browser_monitoring.request_url = None
_settings.browser_monitoring.request_params = []

_settings.transaction_name.limit = None
_settings.transaction_name.naming_scheme = os.environ.get(
        'BLUEWARE_TRANSACTION_NAMING_SCHEME')

_settings.slow_sql.enabled = True

_settings.synthetics.enabled = True

# _settings.btm_hash_value = None
# _settings.btm_url_match = {}
# _settings.rule_index_map = {}
...
def global_settings():
    ...
    >>> global_settings.browser_monitoring.transform = False
    >>> global_settings.browser_monitoring.transform
    False
...
def apply_config_setting(settings_object, name, value):
    ...
    >>> name = 'browser_monitoring.transform'
    >>> value = False
    >>>
    >>> global_settings = global_settings()
...
def create_settings_snapshot(server_side_config={}, settings=_settings):
    ...
    >>> server_config = { 'browser_monitoring.transform': False }
    >>>
