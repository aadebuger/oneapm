...
from blueware.samplers.data_sampler import DataSampler

from blueware.core.config import global_settings_dump, global_settings
from blueware.core.data_collector import create_session
from blueware.network.exceptions import (ForceAgentRestart,
        ForceAgentDisconnect, DiscardDataForRequest, RetryDataForRequest)
from blueware.core.environment import environment_settings
from blueware.core.rules_engine import RulesEngine, SegmentCollapseEngine
from blueware.core.stats_engine import StatsEngine, CustomMetrics
from blueware.core.internal_metrics import (InternalTrace,
        InternalTraceContext, internal_metric)
from blueware.core.xray_session import XraySession
from blueware.core.profile_sessions import profile_session_manager
...
class Application(object):
    ...
    def activate_session(self, timeout=0.0):
        ...
        thread = threading.Thread(target=self.connect_to_data_collector,
                name='OA-Activate-Session/%s' % self.name)
        ...
        if self._detect_deadlock:
            ...
            if not self._deadlock_event.isSet():
                _logger.warning('Detected potential deadlock while waiting '
                        ...
                        'the blueware.agent.register_application() function '
                        'or if necessary report this problem to blueware '
        ...
        try:
                ...
                if active_session:
                    ...
                    try:
                        ...
                        if settings.debug.log_normalization_rules:
                            _logger.debug('The URL normalization rules for '
                                    '%r are %r.', self._app_name,
                                     configuration.url_rules)
                            _logger.debug('The metric normalization rules '
                                    'for %r are %r.', self._app_name,
                                     configuration.metric_name_rules)
                            _logger.debug('The transaction normalization '
                        ...
                        _logger.exception('The agent normalization rules '
                                ...
                                'report this to blueware support for '
                                'investigation.')
                    ...
                    if retries:
                        ...
                        if warning:
                            _logger.warning('Registration of the application '
                                    ...
                                    'report this problem to blueware '
                            ...
                            _logger.error('Registration of the application '
                                    ...
                                    'report this problem to blueware support '
            ...
            try:
                ...
                if not self._agent_shutdown and not self._pending_shutdown:
                    _logger.exception('Unexpected exception when processing '
                            ...
                            'report this problem to blueware support for '
                            'further investigation.')
                else:
                    raise

            # initialize business transaction settings
            try:
                self.update_btm_url_match()
            except Exception as exc:
                _logger.error('Unexpected exception when initialize business transaction.%r',
                              exc)
            ...
            if not self._agent_shutdown and not self._pending_shutdown:
                _logger.exception('Unexpected exception when registering '
                        ...
                        'persists, please report this problem to blueware '
                        'support for further investigation.')
    ...
    def validate_process(self):
        ...
        if self._process_id and process_id != self._process_id:
            _logger.warning('Attempt to reactivate application or record '
                    ...
                    'report this problem to blueware support for further '
    ...
    def normalize_name(self, name, rule_type):
        ...
        try:
            ...
            _logger.exception('The application of the normalization '
                    ...
                    'blueware support for further investigation.', name)
        ...
        with InternalTraceContext(internal_metrics):
            try:
                ...
                _logger.exception('The generation of transaction data has '
                        ...
                        'this problem to blueware support for further '
                        'investigation.')
            ...
            with self._stats_lock:
                try:
                    ...
                    _logger.exception('The merging of transaction data has '
                            ...
                            'Please report this problem to blueware support '
                            'for further investigation.')
    ...
    def start_xray(self, command_id=0, **kwargs):
        ...
        try:
            ...
            _logger.warning('An xray session was requested but appropriate '
                    ...
                    'to blueware support for further investigation. '
    ...
    def cmd_start_profiler(self, command_id=0, **kwargs):
        ...
        if not hasattr(sys, '_current_frames'):
            _logger.warning('A thread profiling session was requested for '
                    ...
                    'Python interpreter being used. Contact blueware '
    ...
    def cmd_stop_profiler(self, command_id=0, **kwargs):
        ...
        if fps is None:
            _logger.warning('A request was received to stop a thread '
                    ...
                    'a regular basis, please report this problem to '
                    'blueware support for further investigation.',
                    self._app_name)
    ...
    def cmd_restart(self, command_id=0, **kwargs):
        """Restart agent."""
        ...
        with InternalTraceContext(internal_metrics):
            with InternalTrace('Supportability/Python/Harvest/Calls/harvest'):
                ...
                for data_sampler in self._data_samplers:
                    try:
                        for sample in data_sampler.metrics():
                            try:
                                ...
                                _logger.exception('The merging of custom '
                                        ...
                                        'the data source provider or '
                                        'blueware support for further '
                        ...
                        _logger.exception('The merging of custom metric '
                                ...
                                'source provider or blueware support for '
                ...
                try:
                    ...
                    if hasattr(configuration, 'btm_value'):
                        try:
                            self.update_btm_match_rules()
                        except Exception as exc:
                           _logger.error('Unexpected exception when update business transaction.%r',
                                         exc)
                    ...
                    if self._period_start != period_end:
                        ...
                        if self._merge_count <= maximum:
                            ...
                            _logger.error('Unable to report main transaction '
                                    ...
                                    'please report this problem to blueware '
                                    'support for further investigation.',
                                    maximum)
    ...
    def _check_url_match(self, method, is_not, operate, url):
        def _url_match(request_method, request_uri):
            if url is None or request_uri is None:
                return False
            ...
            operate_check = {
                'Equals': url == request_uri,
                'In': request_uri in url.split(','),
                'Contains': url in request_uri,
                'StartsWith': request_uri.startswith(url),
                'EndWith': request_uri.endswith(url)
            }

            if is_not:
                return not operate_check[operate]

            return operate_check[operate]
        return _url_match

    def update_btm_url_match(self):
        settings = self.configuration

        if hasattr(settings, 'btm_value') and hasattr(settings.btm_value, 'rules'):

            index = 0
            for rule in settings.btm_value.rules:
                self._rule_index_map[rule['id']] = index
                index += 1

                if not rule['ignore']:
                    rule_match_expression = rule['match_expression']
                    rule_match_expression.sort(key=lambda k: k['match_id'])

                    url_match = self._check_url_match(method=rule_match_expression[0]['match_value'],
                                                      is_not=rule_match_expression[1]['is_not'],
                                                      operate=rule_match_expression[1]['operate'],
                                                      url=rule_match_expression[1]['match_value'])

                    self.btm_url_match.setdefault(url_match, []).append(rule)

    def _btm_rule_binary_search(self, source, dest, key):
        low = 0
        height = len(source) - 1
        while low <= height:
            mid = (low+height) // 2
            if source[mid][key] < dest[key]:
                low = mid + 1
            elif source[mid][key] > dest[key]:
                height = mid - 1
            else:
                return mid
    ...
    def rebuild_btm_rules(self, new_rules, settings):

        will_delete_rule_indexs = []
        will_delete_expression_indexs = []

        for rule in new_rules:
            if not hasattr(settings.btm_value, 'rules'):
                settings.btm_value.rules = []

            index_of_rule = self._rule_index_map.get(rule['id'], -1)
            if index_of_rule == -1:
                settings.btm_value.rules.append(rule)
                continue

            if rule['state'] == 1:
                will_delete_rule_indexs.append(index_of_rule)
                continue

            rule_match_expression = rule['match_expression']
            del rule['match_expression']

            settings.btm_value.rules[index_of_rule].update(rule)

            old_expression = settings.btm_value.rules[index_of_rule]['match_expression']

            for expression in rule_match_expression:
                index_of_exp = self._btm_rule_binary_search(source=old_expression,
                                                            dest=expression,
                                                            key='match_id')
                ...
                if expression['state'] == 1:
                    will_delete_expression_indexs.append((index_of_rule, index_of_exp))
                    continue

                old_expression[index_of_exp].update(expression)

        # delete rules and expressions when it's state has been set to 1
        for rule_index in will_delete_rule_indexs:
            del settings.btm_value.rules[rule_index]

        for rule_index, expression_index in will_delete_expression_indexs:
            del settings.btm_value.rules[rule_index]['match_expression'][expression_index]

        # update btm_url_match, rule_index_map
        self._rule_index_map = {}
        self.btm_url_match = {}
        self.update_btm_url_match()

    def update_btm_match_rules(self):
        btm_match_rules = self._active_session.get_match_rule()
        if btm_match_rules['hash_value'] != self.configuration.btm_value.hash_value:
            self.configuration.btm_value.hash_value = btm_match_rules['hash_value']
            new_rules = btm_match_rules['rules']

            if new_rules:
                self.rebuild_btm_rules(new_rules, self.configuration)
    ...
    def process_agent_commands(self):
        ...
        with self._agent_commands_lock:
            ...
            for command in agent_commands:
                ...
                if cmd_res and self._active_session:
