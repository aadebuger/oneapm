...
import blueware.core.application
...
from blueware.core.thread_utilization import thread_utilization_data_source
...
def check_environment():
    ...
    if 'uwsgi' in sys.modules:
        ...
        if not hasattr(uwsgi, 'version_info'):
            _logger.warning('The blueware Python Agent requires version '
                    ...
                    'in data being reported incorrectly.')
        ...
        if not uwsgi.opt.get('enable-threads'):
            _logger.warning('The blueware Python Agent requires that when '
                    ...
                    'data being reported by the agent.')
...
class Agent(object):
    ...
    created needs to be via the 'blueware.core.config' module.
    ...
    def agent_singleton():
        ...
        settings = blueware.core.config.global_settings()
        ...
        _logger.info('blueware Python Agent (%s)' % blueware.version)

        check_environment()

        if 'BLUEWARE_ADMIN_COMMAND' in os.environ:
            if settings.debug.log_agent_initialization:
                _logger.info('Monitored application started using the '
                        'blueware-admin command with command line of %s.',
                        os.environ['BLUEWARE_ADMIN_COMMAND'])
            else:
    ...
    def __init__(self, config):
        ...
        if self._config.enabled:
            ...
            # join, but our OA background greenlet will never join since it has
    ...
    def global_settings(self):
        ...
        'blueware.core.config' module directly.

        """

        return blueware.core.config.global_settings()
    ...
    def activate_application(self, app_name, linked_applications=[],
        ...
        with self._lock:
            ...
            if not application:
                ...
                application = blueware.core.application.Application(
        ...
        try:
            ...
            if self._process_shutdown:
                _logger.exception('Unexpected exception in main harvest '
                        ...
                        'on a regular basis, then please report it to '
                        'blueware support for further investigation.')

            else:
                _logger.exception('Unexpected exception in main harvest '
                        'loop. Please report this problem to blueware '
    ...
    def shutdown_agent(self, timeout=None):
        ...
        _logger.info('blueware Python Agent Shutdown')
