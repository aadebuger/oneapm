...
import blueware.core.error_collector
import blueware.core.trace_node

from blueware.core.metric import ApdexMetric, TimeMetric
from blueware.core.internal_metrics import internal_trace
from blueware.core.string_table import StringTable
...
class TransactionNode(_TransactionNode):
    ...
    def error_details(self):
        ...
        for error in self.errors:
            ...
            if self.trip_id:
                custom_params['bw.trip_id'] = self.trip_id
            if self.path_hash:
                custom_params['bw.path_hash'] = self.path_hash

            if custom_params:
                params["custom_params"] = custom_params

            yield blueware.core.error_collector.TracedError(
    ...
    @internal_trace('Supportability/Python/TransactionNode/Calls/transaction_trace')
    def transaction_trace(self, stats, limit, connections):

        self.trace_node_count = 0
        self.trace_node_limit = limit

        start_time = blueware.core.trace_node.root_start_time(self)
        ...
        custom_params = self.custom_params or None
        parameter_groups = self.parameter_groups or {}
        ...
        # By prepending 'bw.' to the custom param names, they get treated as
        ...
        if self.synthetics_resource_id:
            custom_params['bw.synthetics_resource_id'] = \
                    self.synthetics_resource_id
            custom_params['bw.synthetics_job_id'] = \
                    self.synthetics_job_id
            custom_params['bw.synthetics_monitor_id'] = \
        ...
        root = blueware.core.trace_node.TraceNode(int(self.start_time * 1000.0),
                int(self.end_time * 1000.0), 'ROOT', {}, [trace_node], label=class_name, method_name=method_name)

        return blueware.core.trace_node.RootNode(start_time=start_time,
