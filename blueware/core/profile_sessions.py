...
from blueware.core.config import global_settings
from blueware.core.transaction_cache import transaction_cache

from blueware.core.internal_metrics import (internal_trace, internal_metric)
...
AGENT_PACKAGE_DIRECTORY = os.path.dirname(blueware.__file__) + '/'
...
def collect_stack_traces(include_bw_threads=False, include_xrays=False):
    ...
    for (txn, thread_id, thread_category, frame) in \
        ...
        # Skip OA Threads unless explicitly requested.

        if (thread_category == 'AGENT') and (not include_bw_threads):
            continue
        ...
        with self._lock:
            ...
            if not self._profiler_thread_running:
                self._profiler_thread = threading.Thread(
                        target=self._profiler_loop, name='OA-Profiler-Thread')
...
class ProfileSession(object):
    ...
    def profile_data(self):
        ...
        for key in flat_tree.keys():
            flat_tree[key].insert(0, {six.u('cpu_time'): 0})
