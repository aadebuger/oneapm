...
from blueware.packages.six import u

import blueware.packages.requests as requests

from blueware.packages.requests import certs

from blueware import version
from blueware.common.encoding_utils import unpack_field
from blueware.core.config import (global_settings, create_settings_snapshot,
        global_settings_dump)
from blueware.core.internal_metrics import (internal_trace, InternalTrace,
        internal_metric)

from blueware.network.exceptions import (NetworkInterfaceException,
...
USER_AGENT = 'BLUEWARE-PythonAgent/%s (Python %s %s)' % (
...
def proxy_server():
    ...
    if settings.log_level in _LOG_LEVEL_TEMP:
        settings.log_level = _LOG_LEVEL_TEMP[settings.log_level]
    else:
...
@patch_function_wrapper(
        'blueware.packages.requests.packages.urllib3.connectionpool',
...
@patch_function_wrapper(
        'blueware.packages.requests.adapters',
        'HTTPAdapter.request_url')
def _requests_request_url_workaround(wrapped, instance, args, kwargs):
    from blueware.packages.requests.adapters import urldefragauth
...
def send_request(session, url, method, license_key, agent_run_id=None,
    ...
    if btm_hash_value is not None:
        params['hash_value'] = btm_hash_value

    headers['User-Agent'] = USER_AGENT
    headers['Content-Encoding'] = 'identity'

    if settings.proxy_host:
        auth = 'blueware:blueware'
        headers['Proxy-Authorization'] = ''.join(['Basic ', base64.b64encode(auth)])
    ...
    try:
        ...
        _logger.exception('Error encoding data for JSON payload for '
                ...
                'to blueware support.', method, payload)
    ...
    if r.status_code == 400:
        _logger.error('Data collector is indicating that a bad '
                ...
                'problem to blueware support.', url, headers, params,
                payload)
        ...
        _logger.warning('Data collector is indicating that a request for '
                ...
                'regular basis, please report this problem to blueware '
        ...
        _logger.warning('Data collector is indicating that it was sent '
                ...
                'on a regular basis, please report this problem to '
                'blueware support for further investigation.', method)
        ...
        _logger.warning('Data collector is unavailable. This can be a '
                ...
                'problem to blueware support for further investigation.')
        ...
        if not settings.proxy_host or not settings.proxy_port:
            ...
            _logger.warning('An unexpected HTTP response was received from '
                    ...
                    'persists then please report this problem to blueware '
    ...
    try:
        ...
        _logger.exception('Error decoding data for JSON payload for '
                'method %r with payload of %r. Please report this problem '
                'to blueware support.', method, content)
    ...
    if error_type == 'BLUEWARE::Agent::LicenseException':
        _logger.error('Data collector is indicating that an incorrect '
                ...
                'blueware support.', license_key)

        raise DiscardDataForRequest(message)

    elif error_type == 'BLUEWARE::Agent::PostTooBigException':
    ...
    if error_type == 'BLUEWARE::Agent::ForceRestartException':
    ...
    elif error_type == 'BLUEWARE::Agent::ForceDisconnectException':
        _logger.critical('Disconnection of the agent has been requested by '
                ...
                'disconnection is %r. Please contact blueware support '
    ...
    _logger.warning('An unexpected server error was received from the '
            ...
            'then please report this problem to blueware support for '
...
def _convert_server_config(server_config):
    target = {}
    settings = global_settings()

    settings_list_type = ('ignored_params', 'error_collector.ignore_errors')

    for name, value in server_config.items():
        if hasattr(settings, name) and type(getattr(settings, name)) == type(value):
            target[name] = value
            continue
        ...
        if isinstance(value, dict):
            for key, item in value.items():
                key = '.'.join([name, key])
                if key in settings_list_type:
                    target[key] = item.split(',')
                else:
                    target[key] = item
        elif name in settings_list_type:
            target[name] = value.split(',')
        else:
            target[name] = value
...
class ApplicationSession(object):
    ...
    def send_request(cls, session, url, method, license_key,
            agent_run_id=None, payload=(), btm_hash_value=None):
        return send_request(session, url, method, license_key,
            agent_run_id, payload, btm_hash_value)
    ...
    def shutdown_session(self):
        ...
        _logger.info('Successfully shutdown blueware Python agent '
    ...
    @internal_trace('Supportability/Python/Collector/Calls/transaction_sample_data')
    ...
    @internal_trace('Supportability/Python/Collector/Calls/get_agent_commands')
    ...
    @internal_trace('Supportability/Python/Collector/Calls/send_agent_command_results')
    ...
    @internal_trace('Supportability/Python/Collector/Calls/get_match_rule')
    def get_match_rule(self):
        """Receive btm match rules from the data collector.

        """

        return self.send_request(self.requests_session, self.collector_url,
                'get_match_rule', self.license_key, self.agent_run_id,
                btm_hash_value=self.configuration.btm_value.hash_value)
    ...
    def create_session(cls, license_key, app_name, linked_applications,
        ...
        if not license_key:
            _logger.error('A valid account license key cannot be found. '
                ...
                'file or via the BLUEWARE_LICENSE_KEY environment variable?')
        ...
        try:
            ...
            connect_settings['browser_monitoring.is_js_text'] = (
                    settings['browser_monitoring.is_js_text'])
            ...
            server_config = _convert_server_config(server_config)
            ...
            _logger.info('Successfully registered Blueware Python agent '
...
_developer_mode_responses = {
    'get_redirect_host': u('tpm.oneapm.com:443'),

    'agent_settings': [],

    'connect': {
        u('js_agent_loader'): u('<!-- NREUM -->'),
        u('js_agent_file'): u('http://tpm.oneapm.com/static/js/bw-loader.js'),
        u('browser_key'): u('1234567890'),
        u('browser_monitoring.loader_version'): u('0'),
        u('beacon'): u('tpm.oneapm.com'),
        u('error_beacon'): u('tpm.oneapm.com'),
        u('apdex_t'): 0.5,
...
def create_session(license_key, app_name, linked_applications,
    ...
    for key, value in list(six.iteritems(application_settings)):
        ...
        if key == 'transaction_tracer.transaction_threshold' and value is None:
            value = 'apdex_f'
        ...
        if (not isinstance(value, six.string_types) and
            ...
            if isinstance(value, list):
                try:
                    application_settings[key] = ','.join(value)
                except:
    ...
    try:
        ...
        value = session.configuration.transaction_tracer.transaction_threshold
        try:
            session.configuration.transaction_tracer.transaction_threshold = float(value)
        except:
            session.configuration.transaction_tracer.transaction_threshold = None
