...
def usage(name):
    ...
    print('Usage: blueware-admin %s %s' % (name, details.options))

@command('help', '[command]', hidden=True)
def help(args):
    if not args:
        print('Usage: blueware-admin command [options]')
        print()
        print("Type 'blueware-admin help <command>'", end='')
        ...
        if name not in _commands:
            ...
            print("Type 'blueware-admin help' for usage.")
...
def setup_log_intercept():
    ...
    class FilteredStreamHandler(logging.StreamHandler):
        def emit(self, record):
            ...
            if record.name.startswith('blueware.packages'):
                return
    ...
    _stdout_logger = logging.getLogger('blueware')
