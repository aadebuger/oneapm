...
supplied by the environment variable BLUEWARE_CONFIG_FILE. Alternatively,
just the licence key, application and log file details can be supplied via
environment variables BLUEWARE_LICENSE_KEY, BLUEWARE_APP_NAME and
BLUEWARE_LOG.""")
def run_python(args):
    import os
    import sys
    ...
    def log_message(text, *args):
        if startup_debug:
            ...
            print('BLUEWARE: %s (%d) - %s' % (timestamp, os.getpid(), text))

    log_message('blueware Admin Script (%s)', __file__)
    ...
    from blueware import version, __file__ as root_directory
    ...
    os.environ['BLUEWARE_ADMIN_COMMAND'] = repr(sys.argv)

    os.environ['BLUEWARE_PYTHON_PREFIX'] = os.path.realpath(
            os.path.normpath(sys.prefix))
    os.environ['BLUEWARE_PYTHON_VERSION'] = '.'.join(
