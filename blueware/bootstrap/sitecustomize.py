...
startup_debug = os.environ.get('BLUEWARE_STARTUP_DEBUG',
...
def log_message(text, *args):
    if startup_debug:
        ...
        sys.stdout.write('BLUEWARE: %s (%d) - %s\n' % (timestamp,
                os.getpid(), text))
        sys.stdout.flush()

log_message('blueware Bootstrap (%s)', __file__)
...
for name in sorted(os.environ.keys()):
    if name.startswith('BLUEWARE_') or name.startswith('PYTHON'):
...
# in the same Python installation as the original blueware-admin script
# which was run and only continue if we are.

expected_python_prefix = os.environ.get('BLUEWARE_PYTHON_PREFIX')
actual_python_prefix = os.path.realpath(os.path.normpath(sys.prefix))

expected_python_version = os.environ.get('BLUEWARE_PYTHON_VERSION')
...
if python_prefix_matches and python_version_matches:
    ...
    license_key = os.environ.get('BLUEWARE_LICENSE_KEY', None)
    ...
    if license_key or config_file:
        ...
        # 'blueware' module to fail. What we do is see if the root
        ...
        # 'blueware' module to reduce chance that will cause any issues.
        ...
        log_message('agent_version = %r', blueware.version)
        ...
        blueware.agent.initialize(config_file, environment)
