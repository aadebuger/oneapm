...
from blueware.agent import (FunctionTrace, WebTransaction,
...
def record_exception(transaction, exc_info):
    ...
    if exc is cyclone.web.HTTPError:
...
def request_environment(application, request):
    # This creates a WSGI environ dictionary from a cyclone request.

    result = getattr(request, '_bw_request_environ', None)
    ...
    request._bw_request_environ = result = {}

    result['REQUEST_URI'] = request.uri
    result['QUERY_STRING'] = request.query

    value = request.headers.get('X-BLUEWARE-ID')
    if value:
        result['HTTP_X_BLUEWARE_ID'] = value

    value = request.headers.get('X-BLUEWARE-Transaction')
    if value:
        result['HTTP_X_BLUEWARE_TRANSACTION'] = value
...
def retrieve_transaction_request(transaction):
    ...
    if hasattr(transaction, '_bw_current_request'):
        if transaction._bw_current_request is not None:
            return transaction._bw_current_request()

def retrieve_request_transaction(request):
    # Retrieves any transaction already associated with the request.

    return getattr(request, '_bw_transaction', None)
...
def request_finished(request):
    ...
    return getattr(request, '_bw_request_finished', None)

def request_caller_context(request):
    # Returns the most recently specified named caller context.

    if not hasattr(request, '_bw_caller_context'):
        return

    if not request._bw_caller_context:
        return

    return request._bw_caller_context[-1]

@contextlib.contextmanager
def _bw_wrapper_named_caller_context_(request, name):
    if not hasattr(request, '_bw_caller_context'):
        request._bw_caller_context = []

    request._bw_caller_context.append(name)

    yield

    request._bw_caller_context.pop(-1)
...
def initiate_request_monitoring(request):
    ...
    transaction._bw_current_request = weakref.ref(request)

    # Record framework information for generation of framework metrics.

    import cyclone

    if hasattr(cyclone, 'version'):
        version = '.'.join(map(str, cyclone.version))
    else:
        version = None

    transaction.add_framework_info('Cyclone/ASYNC', version)

    return transaction

def suspend_request_monitoring(request, name, group='Python/Cyclone',
    ...
    if transaction is None:
        _logger.error('Runtime instrumentation error. Suspending the '
                'Cyclone transaction but there was no transaction cached '
                'against the request object. Report this issue to blueware '
    ...
    if request._bw_wait_function_trace:
        _logger.error('Runtime instrumentation error. Suspending the '
                'Cyclone transaction when it has already been suspended. '
                'Report this issue to blueware support.\n%s',
    ...
    if rollup is None:
        if cyclone_settings.instrumentation.metrics.async_wait_rollup:
            rollup = 'Async Wait'
...
def finish_request_monitoring(request):
    ...
    request._bw_request_finished = True
...
def finalize_request_monitoring(request, exc=None, value=None, tb=None):
    ...
    try:
        ...
        transaction._bw_current_request = None

        request._bw_transaction = None
        request._bw_wait_function_trace = None
        request._bw_request_finished = True
