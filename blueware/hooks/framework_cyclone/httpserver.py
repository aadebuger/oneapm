...
def _bw_wrapper_HTTPConnection__on_headers_(wrapped, instance, args, kwargs):
    ...
    if transaction is not None:
        _logger.error('Runtime instrumentation error. Starting a new '
                'Cyclone web request but there is a transaction active '
                'already. Report this issue to blueware support.\n%s',
...
def _bw_wrapper_HTTPConnection__on_request_body_(wrapped, instance,
        args, kwargs):
    ...
    try:
        ...
        elif request.connection._request_finished:
            finalize_request_monitoring(request)
...
def _bw_wrapper_HTTPConnection__finish_request(wrapped, instance,
        args, kwargs):
