...
from blueware.agent import (ExternalTrace, ObjectWrapper, current_transaction)
...
def httplib_putheader_wrapper(wrapped, instance, args, kwargs):
    ...
    # Remember if we see any OA headers being set. This is only doing
    # it if we see either, but they should always both be getting set.

    def bw_header(header, *args, **kwargs):
        return header.upper() in ('X-BLUEWARE-ID', 'X-BLUEWARE-TRANSACTION')

    connection = instance

    if bw_header(*args, **kwargs):
        connection._bw_skip_headers = True
