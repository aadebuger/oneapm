from blueware.agent import (current_transaction, wrap_object, DatabaseTrace,
...
class ConnectionWrapper(DBAPI2ConnectionWrapper):
        ...
        with FunctionTrace(transaction, name):
            if exc is None and value is None and tb is None:
                with DatabaseTrace(transaction, 'COMMIT',
                        self._bw_dbapi2_module, self._bw_connect_params):
    ...
    def executemany(self, sql, seq_of_parameters):
        transaction = current_transaction()
        with DatabaseTrace(transaction, sql, self._bw_dbapi2_module,
                self._bw_connect_params, None, list(seq_of_parameters)[0]):
