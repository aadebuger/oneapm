...
def _bw_wrapper_Flask_add_url_rule_input_(wrapped, instance, args, kwargs):
    ...
    if view_func is not None:
        view_func = _bw_wrapper_handler_(view_func)

    return wrapped(rule, endpoint, view_func, **options)

@function_wrapper
def _bw_wrapper_endpoint_(wrapped, instance, args, kwargs):
    def _bind_params(f, *args, **kwargs):
        return f

    f = _bind_params(*args, **kwargs)

    return wrapped(_bw_wrapper_handler_(f))

def _bw_wrapper_Flask_endpoint_(wrapped, instance, args, kwargs):
    return _bw_wrapper_endpoint_(wrapped(*args, **kwargs))
...
@function_wrapper
def _bw_wrapper_error_handler_(wrapped, instance, args, kwargs):
    transaction = current_transaction()
...
def _bw_wrapper_Flask__register_error_handler_(wrapped, instance, args, kwargs):
    def _bind_params(key, code_or_exception, f):
        return key, code_or_exception, f

    key, code_or_exception, f = _bind_params(*args, **kwargs)

    f = _bw_wrapper_error_handler_(f)

    return wrapped(key, code_or_exception, f)

def _bw_wrapper_Flask_try_trigger_before_first_request_functions_(
...
def _bw_wrapper_Flask_before_first_request_(wrapped, instance, args, kwargs):
...
def _bw_wrapper_Flask_before_request_(wrapped, instance, args, kwargs):
    def _params(f, *args, **kwargs):
        return f, args, kwargs

    f, _args, _kwargs = _params(*args, **kwargs)
    f = _bw_wrapper_Flask_before_request_wrapped_(f)

    return wrapped(f, *_args, **_kwargs)

def _bw_wrapper_Flask_after_request_(wrapped, instance, args, kwargs):
    def _params(f, *args, **kwargs):
        return f, args, kwargs
...
def _bw_wrapper_Flask_teardown_request_(wrapped, instance, args, kwargs):
    def _params(f, *args, **kwargs):
        return f, args, kwargs
...
def _bw_wrapper_Flask_teardown_appcontext_(wrapped, instance, args, kwargs):
    def _params(f, *args, **kwargs):
        return f, args, kwargs
...
def _bw_wrapper_Blueprint_endpoint_(wrapped, instance, args, kwargs):
    return _bw_wrapper_endpoint_(wrapped(*args, **kwargs))

@function_wrapper
def _bw_wrapper_Blueprint_before_request_wrapped_(wrapped, instance,
        args, kwargs):
...
def _bw_wrapper_Blueprint_before_request_(wrapped, instance, args, kwargs):
    def _params(f, *args, **kwargs):
        return f, args, kwargs

    f, _args, _kwargs = _params(*args, **kwargs)
    f = _bw_wrapper_Blueprint_before_request_wrapped_(f)

    return wrapped(f, *_args, **_kwargs)

def _bw_wrapper_Blueprint_before_app_request_(wrapped, instance,
        args, kwargs):
...
def _bw_wrapper_Blueprint_before_app_first_request_(wrapped, instance,
        args, kwargs):
...
def _bw_wrapper_Blueprint_after_request_(wrapped, instance, args, kwargs):
    def _params(f, *args, **kwargs):
        return f, args, kwargs
...
def _bw_wrapper_Blueprint_after_app_request_(wrapped, instance, args, kwargs):
    def _params(f, *args, **kwargs):
        return f, args, kwargs
...
def _bw_wrapper_Blueprint_teardown_request_(wrapped, instance, args, kwargs):
    def _params(f, *args, **kwargs):
        return f, args, kwargs
...
def _bw_wrapper_Blueprint_teardown_app_request_(wrapped, instance,
        args, kwargs):
