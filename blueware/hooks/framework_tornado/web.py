...
def _bw_wrapper_RequestHandler__execute_method_(wrapped, instance,
        args, kwargs):
...
def _bw_wrapper__RequestDispatcher_execute(wrapped, instance, args, kwargs):
    # This variant of the Application.__call__() wrapper is used when it
    # is believed that we are being called for a HTTP request where
    # there is no request content. There should be no transaction
    # associated with the Tornado request object and also no current
...
def _bw_wrapper__RequestDispatcher_set_request(wrapped, instance, args, kwargs):
    ...
    # set connection._bw_request for _finish_request in http1connection
    request.connection._bw_request = request
