...
class GeneratorState(object):
    ...
    def __init__(self, first_yielded=False):
        self.first_yielded = first_yielded

def _bw_wrapper_gen_coroutine_generator_(generator, state, first_yield=None, is_wrapped_gen_inited=False):
    ...
    try:
        ...
        while True:
            ...
            if is_wrapped_gen_inited:
                try:
                    if state.first_yielded:
                        state.first_yielded = False
...
def _bw_wrapper_gen_Runner___init___(wrapped, instance, args, kwargs):
    ...
    # add support for Tornado 4.0+
    if hasattr(instance, 'handle_yield'):
        state.first_yielded = True
        is_wrapped_gen_inited = True

        def _params(gen, result_future, first_yield, *args, **kwargs):
            return gen, result_future, first_yield, args, kwargs

        generator, result_future, first_yield, _args, _kwargs = _params(*args, **kwargs)
        generator = _bw_wrapper_gen_coroutine_generator_(generator, state, first_yield, is_wrapped_gen_inited)
        generator.send(None)

        return wrapped(generator, result_future, first_yield, *_args, **_kwargs)
    ...
    generator = _bw_wrapper_gen_coroutine_generator_(generator, state)

    return wrapped(generator, *_args, **_kwargs)

@function_wrapper
def _bw_wrapper_gen_coroutine_function_(wrapped, instance, args, kwargs):
...
def _bw_wrapper_gen_coroutine_(wrapped, instance, args, kwargs):
    ...
    return _bw_wrapper_gen_coroutine_function_(wrapped(*args, **kwargs))
