...
from blueware.core.agent import agent_instance
from blueware.core.config import global_settings, flatten_settings
from blueware.core.transaction_cache import transaction_cache
from blueware.common.object_wrapper import ObjectWrapper
...
class OutputWrapper(ObjectWrapper):
    ...
    def flush(self):
        try:
            ...
            return self._bw_next_object.flush()
    ...
    def write(self, data):
        try:
            ...
            return self._bw_next_object.write(data)
    ...
    def writelines(self, data):
        try:
            ...
            return self._bw_next_object.writelines(data)
...
class ConsoleShell(cmd.Cmd):
    ...
    def do_prompt(self, flag=None):
        ...
        if flag == 'on':
            self.prompt = '(blueware:%d) ' % os.getpid()
        elif flag == 'off':
            self.prompt = ''
    ...
    def do_import_hooks(self):
        ...
        from blueware.config import module_import_hook_results
...
class ClientShell(cmd.Cmd):
    ...
    def __init__(self, config_file, stdin=None, stdout=None, log=None):
        ...
        listener_socket = self.__config_object.get('blueware',
...
def main():
    if len(sys.argv) == 1:
        print("Usage: blueware-console config_file")
        sys.exit(1)
