    ...
    def upload(self, session):
        ...
        try:
            ...
            _logger.exception('The processing of custom metrics from '
                    ...
                    'provider or blueware support for further '
        ...
        try:
            ...
            _logger.exception('Unable to report data custom metrics '
                    ...
                    'the data source provider or blueware support for '
...
class Agent(object):
    ...
    def run(self):
        ...
        _logger.info('blueware Python Agent - Data Source (%s)',
                agent_version)
        ...
        for (source, name, settings, properties) in self._data_sources:
            try:
                data_sampler = DataSampler('blueware (Platform)', source,
...
def run(config_file, background=False):
    ...
    def option(name, section='blueware', type=None, **kwargs):
        try:
    ...
    license_key = os.environ.get('BLUEWARE_LICENSE_KEY')
    license_key = option('license_key', default=license_key)

    host = option('host', default='tpm.oneapm.com')
    ...
    log_file = os.environ.get('BLUEWARE_LOG_FILE')
    log_file = option('log_file', default=log_file)

    if log_file in ('stdout', 'stderr'):
        log_file = None

    log_level = os.environ.get('BLUEWARE_LOG_LEVEL', 'INFO').upper()
